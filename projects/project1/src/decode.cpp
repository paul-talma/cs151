...
static const std::unordered_map<Opcode, InstType> sc_instTable = {
    {Opcode::R, InstType::R},     {Opcode::L, InstType::I},
    {Opcode::I, InstType::I},     {Opcode::S, InstType::S},
    {Opcode::B, InstType::B},     {Opcode::LUI, InstType::U},
    {Opcode::AUIPC, InstType::U}, {Opcode::JAL, InstType::J},
    {Opcode::JALR, InstType::I},  {Opcode::SYS, InstType::I},
...
enum Constants {
    ...
    shift_csr_imm = shift_rs1,
    shift_func2 = shift_rs2 + width_reg,
    shift_func7 = shift_rs2 + width_reg,
    shift_s_imm_low = width_opcode,
    shift_s_imm_high = shift_rs2,
    ...
    mask_csr_imm = (1 << width_csr_imm) - 1,
    mask_s_imm_low = (1 << width_s_imm_low) - 1,
    mask_s_imm_high = ((1 << width_s_imm_high) - 1) << width_s_imm_low,
    mask_u_imm = (1 << width_u_imm) - 1,
};
...
static const char *op_string(const Instr &instr) {
    ...
    case Opcode::R:
        ...
        case 0:
            ...
            std::abort();
        case 1:
            return (func7 & 0x1) ? "MULH" : "SLL";
        case 2:
            return (func7 & 0x1) ? "MULHSU" : "SLT";
        case 3:
            return (func7 & 0x1) ? "MULHU" : "SLTU";
        case 4:
            return "XOR";
        case 5:
            return (func7 & 0x20) ? "SRA" : "SRL";
        case 6:
            return "OR";
        case 7:
            return "AND";
...
std::shared_ptr<Instr> Core::decode(uint32_t instr_code) const {
    ...
    if (op_it == sc_instTable.end()) {
        std::cout << std::hex << "Error: invalid opcode: 0x"
    ...
    case InstType::I: {
        ...
        case Opcode::I:
            ...
            // TODO:
            // sign-extend
            imm = ((instr_code >> 31 & 0x1)
                       ? (instr_code >> shift_i_imm | ~mask_i_imm)
                       : instr_code >> shift_i_imm);
            break;
        case Opcode::L:
        case Opcode::JALR: {
            exe_flags.use_rd = 1;
            ...
            exe_flags.alu_s2_imm = 1;
            // TODO:
            // sign-extend, multiply by 2
            imm = ((instr_code >> 31 & 0x1)
                       ? (instr_code >> 19 | ~(mask_i_imm << 1 | 0x1))
                       : (instr_code >> 20) << 1);
        } break;
        case Opcode::SYS: {
            exe_flags.use_imm = 1;
            ...
            // zero-extend
            imm = (instr_code >> shift_csr_imm) & mask_csr_imm;
        } break;
        case Opcode::FENCE:
            break;
        default:
    ...
    case InstType::S: {
        ...
        exe_flags.alu_s2_imm = 1;
        // todo:
        imm = ((instr_code >> shift_s_imm_low) & mask_s_imm_low) |
              ((instr_code >> shift_s_imm_high) & mask_s_imm_high);
    } break;
    ...
    case InstType::B: {
        ...
        exe_flags.use_imm = 1;
        exe_flags.alu_s2_imm = 1;
        imm = 0 | ((instr_code >> 8) & 0xf) << 1 |
              ((instr_code >> 25) & mask_reg) << 5 |
              ((instr_code >> 7) & 0x1) << 11 | ((instr_code >> 31) << 12);
    } break;
    ...
    case InstType::U: {
        ...
        exe_flags.alu_s2_imm = 1;
        imm = ((instr_code >> 31 & 0x1)
                   ? (instr_code >> shift_u_imm | ~mask_u_imm)
                   : instr_code >> shift_u_imm);
    ...
    case InstType::J: {
        ...
        exe_flags.use_imm = 1;
        exe_flags.alu_s2_imm = 1;
        imm = ((instr_code >> 21) & 0x3ff) << 1 |
              ((instr_code >> 20) & 0x1) << 11 | instr_code & (0xff << 12) |
              (instr_code >> 31) << 20;
    } break;
