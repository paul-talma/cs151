...
Core::Core(const SimContext &ctx, uint32_t core_id, ProcessorImpl *processor)
    : SimObject(ctx, "core"), core_id_(core_id), processor_(processor),
      reg_file_(NUM_REGS) {
    this->reset();
}
...
void Core::if_stage() {
    ...
    DT(2, "IF: instr=0x" << instr_code << ", PC=0x" << std::hex << PC_
                         << std::dec << " (#" << uuid << ")");
    ...
    // lock this stage until branch resolution
    fetch_stalled_ = true;
...
void Core::id_stage() {
    ...
    if (instr->getBrOp() == BrOp::JAL && !instr->getExeFlags().is_exit) {
...
void Core::ex_stage() {
    ...
    // ALU operations
    auto result = this->alu_unit(*stage_data.instr, stage_data.rs1_data,
                                 stage_data.rs2_data, stage_data.PC);

    // Branch operations
    result = this->branch_unit(*stage_data.instr, stage_data.rs1_data,
                               stage_data.rs2_data, result, stage_data.PC);

    DT(2, "EX: result=0x" << std::hex << result << std::dec << " (#"
                          << stage_data.uuid << ")");

    // move instruction data to next stage
    ex_mem_.push({stage_data.instr, stage_data.rs1_data, stage_data.rs2_data,
                  result, stage_data.PC, stage_data.uuid});
    id_ex_.pop();
}
...
void Core::mem_stage() {
    ...
    auto result = this->mem_access(*stage_data.instr, stage_data.result,
                                   stage_data.rs2_data);

    DT(3, "MEM: result=0x" << std::hex << result << std::dec << " (#"
                           << stage_data.uuid << ")");
...
void Core::regfile_read(const Instr &instr, uint32_t *rs1_data,
                        uint32_t *rs2_data) {
    auto exe_flags = instr.getExeFlags();
    ...
    if (exe_flags.use_rs1 && instr.getRs1() != 0) {
        if (!this->data_forwarding(instr.getRs1(), &_rs1_data)) {
            ...
            DT(2, "Regfile: addr=" << instr.getRs1() << ", data=0x" << std::hex
                                   << _rs1_data << std::dec << " (#"
                                   << if_id_.data().uuid << ")");
        }
    }
    ...
    if (exe_flags.use_rs2 && instr.getRs2() != 0) {
        if (!this->data_forwarding(instr.getRs2(), &_rs2_data)) {
            ...
            DT(2, "Regfile: addr=" << instr.getRs2() << ", data=0x" << std::hex
                                   << _rs2_data << std::dec << " (#"
                                   << if_id_.data().uuid << ")");
        }
    }
    ...
    if (exe_flags.is_csr) {
        ...
        DT(2, "CSR: addr=0x" << std::hex << instr.getImm() << ", data=0x"
                             << _rs2_data << std::dec << " (#"
                             << if_id_.data().uuid << ")");
    }
...
void Core::attach_ram(RAM *ram) { mmu_.attach(*ram, 0, 0xFFFFFFFF); }

void Core::showStats() {
    std::cout << std::dec << "PERF: instrs=" << perf_stats_.instrs
              << ", cycles=" << perf_stats_.cycles << std::endl;
}
