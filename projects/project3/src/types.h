...
inline std::ostream &operator<<(std::ostream &os, const FUType& type) {
  switch (type) {
  case FUType::NONE: os << "NONE"; break;
  case FUType::ALU: os << "ALU"; break;
  case FUType::BRU: os << "BRU"; break;
...
enum class MemDataType : uint32_t {
  LB  = 0,
  LH  = 1,
  LW  = 2,
  LBU = 4,
...
inline bool is_rob_younger(uint32_t rob_tag, uint32_t base_tag) {
  auto size2 = ROB_SIZE * 2u;
  auto mask2 = size2 - 1u;
  auto diff = (rob_tag - base_tag) & mask2;
  return diff > 0 && diff < ROB_SIZE;
}
